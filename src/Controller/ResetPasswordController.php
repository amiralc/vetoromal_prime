...

{
    ...
    public function reset(Request $request, UserPasswordEncoderInterface $userPasswordEncoder, TranslatorInterface $translator, string $token = null): Response
    {
        ...
        try {
            $membre = $this->resetPasswordHelper->validateTokenAndFetchUser($token);
    ...
    {
        $membre = $this->entityManager->getRepository(Membre::class)->findOneBy([
            'email' => $emailFormData,
        ]);
        ...
        try {
            $resetToken = $this->resetPasswordHelper->generateResetToken($membre);
        } catch (ResetPasswordExceptionInterface $e) {
        ...
        $email = (new TemplatedEmail())
            ->from(new Address('talyfloramarie-dominique.doua@esprit.tn:', 'Doua Flora'))
            ->to($membre->getEmail())
